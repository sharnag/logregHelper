...
A new class `listGLM` is introduced which is essentially a list of fitted `glm` models.
...
Logistic regression assumes linearity between the logit of the predicted probabilities and each continuous predictors. The `plotPredictors` function assists in this inspection by plotting the numerical predictor against the logit of the response. Both continuous and discrete predictors are also included in the plot.
...
- The S3 methods calls an RCPP function (`rcpp_multipleLRTest`), which performs the Likelihood Ratio test.  
